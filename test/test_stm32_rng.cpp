/*
 * Test cases for STM32WL55 hardware RNG
 */
...
void test_rng_initialization(void) {
    if (!rng_initialized) {
        CryptRNG.begin("STM32 Test Suite");
...
void test_rng_quality(void) {
    // STM32WL55 has hardware TRNG
    TEST_MESSAGE("Running on STM32WL55 with hardware TRNG");
...
void test_platform_not_supported(void) {
    TEST_IGNORE_MESSAGE("STM32WL55 RNG tests only run on STM32WL55 hardware");
}
